...
var len_x = len_y = len_z = 10000; //各軸初期長さ


function makeGcode(){
  len_x = len_y = len_z = wire_length;
  Gcode_Start();
  ...
  for(var t=0; t<Times; t++){ //重ねるレイヤー数
    for(var i=0; i<X/G_Width; i++){
      ...
      content += ["G1 X" + len_x + " Y" + len_y + " Z" + len_z + " E" + ESpeed + "\n",
                  "G92 E0\n"].join("");
    }
...
function movement_calculater(before_point, after_point){
  //X
    ...
    len_x = Math.round((len_x + move_x) * 100) / 100;
  ...
  //Y
    ...
    len_y = Math.round((len_y + move_y) * 100) / 100;
  ...
  //Z
    ...
    len_z = Math.round((len_z + move_z) * 100) / 100;
...
function Gcode_Start(){  //Gcode start
  content += [ "G90\n",  //Set to Absolute Positioning
              ...
              "G92 X" + len_x + " Y" + len_y + " Z" + len_z + " E0\n"].join(""); //set max length
  ...
  content += ["G1 X" + len_x + " Y" + len_y + " Z" + len_z + " F1000 ;Move to new Origin\n",
